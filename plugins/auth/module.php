...
class mysql {
	...
	function query($query, $type, $num) {
		if ($q=mysql_query($query)) {
			...
			//print 'Mysql error: '.mysql_error(); //debug only
			return false;
		}
	}
	...
	class auth {
		...
		function check_new_user($login, $passwd, $passwd2, $mail) {
			...
			if ($db->query("SELECT * FROM users WHERE name='".$login."';", 'num_row', '')!=0) $error[]='Пользователь с таким именем уже существует';
		...
		function reg($login, $passwd, $passwd2, $mail) {
			if (($this->check_new_user($login, $passwd, $passwd2, $mail))=='good') {
				...
				$login = $db->screening($login);
				if ($db->query("INSERT INTO `users` (`id`, `name`, `password`, `email`) VALUES (NULL, '".$login."', '".$passwd."', '".$mail."');", '', '')) return true;
				else {
				...
				if (isset($_COOKIE['id_user']) and isset($_COOKIE['code_user'])) {
					...
					if ($db->query("SELECT * FROM `session` WHERE `id_user`=".$id_user.";", 'num_row', '')==1) {
						...
						if ($data['code_sess']==$code_user and $data['user_agent_sess']==$_SERVER['HTTP_USER_AGENT']) {
							...
							$_SESSION['login_user']=$db->query("SELECT name FROM `users` WHERE  `id` = '".$id_user."';", 'result', 0);
							//~ обновляем куки
		...
		function authorization() {
			...
			if ($db->query("SELECT * FROM `users` WHERE  `name` =  '".$login."' AND  `password` = '".$passwd."';", 'num_row', '')==1) {
				//~ пользователь найден в бд, логин совпадает с паролем
				$_SESSION['id_user']=$db->query("SELECT * FROM `users` WHERE  `name` =  '".$login."' AND  `password` = '".$passwd."';", 'result', 0);
				...
				if ($db->query("SELECT * FROM  `users` WHERE  `name` =  '".$login."';", 'num_row', 0)==1) $error[]='Введен не верный пароль';
		...
		function recovery_pass($login, $mail) {
			$db = new mysql(); //~ создаем новый объект класса
			$login = $db->screening($login);
			$db_inf = $db->query("SELECT * FROM `users` WHERE `name`='".$login."';", 'accos', '');
			if ($db->query("SELECT * FROM `users` WHERE `name`='".$login."';", 'num_row', '')!=1) {
